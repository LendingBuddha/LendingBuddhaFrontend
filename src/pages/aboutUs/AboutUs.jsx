...
const AboutUs = () => {
  const [expandedSections, setExpandedSections] = useState(Array(5).fill(false));
  ...
  const sections = [
    {
      title: 'Our Mission',
      content:
        'At Lending Buddha, our mission is to revolutionize the personal loans market in India by providing a one-stop solution for borrowers with diverse profiles and needs. We aim to facilitate access to both retail and institutional lenders with varying risk appetites, empowering borrowers to find the best possible financing options.',
    },
    {
      title: 'Our Platform',
      content: (
        <ul>
          <li>Interact and discuss business prospects.</li>
          <li>Evaluate all their options before finalizing a deal.</li>
          <li>Benefit from a streamlined process that cuts down on unnecessary intermediaries.</li>
        </ul>
      ),
    },
    {
      ...
      content: (
        <>
          <p>
            <strong>Transparency:</strong> We prioritize clear and open
            communication between borrowers and lenders, ensuring all parties
            have the information they need to make informed decisions.
          </p>
          <p>
            <strong>Efficiency:</strong> By removing intermediaries, we reduce
            the time and cost associated with traditional lending processes.
          </p>
          <p>
            <strong>Security:</strong> Our platform employs robust security
            measures to protect user data and ensure safe transactions.
          </p>
        </>
      ),
    },
      ...
      {sections.map((section, index) => (
        <div key={index} className="about-us-section">
          <div className="section-header" onClick={() => toggleSection(index)}>
            <h2>{section.title}</h2>
            <ExpandMoreIcon className={expandedSections[index] ? 'expanded' : ''} />
          </div>
